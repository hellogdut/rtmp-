...
configure_make()
{
   ...
   if [ "$ARCH" == "android-armv7" ]; then
       ...
       NDK_FLAGS="--platform=android-19 --toolchain=arm-linux-androideabi-4.8 --install-dir=`pwd`/android-toolchain"
       ...
       NDK_FLAGS="--platform=android-19 --toolchain=x86-4.8 --install-dir=`pwd`/android-toolchain"
   fi
   ...
   export CFLAGS=" ${ARCH_FLAGS} -fpic -pie -fPIE -ffunction-sections -funwind-tables -fstack-protector -fno-strict-aliasing -finline-limit=64 "
   export LDFLAGS=" ${ARCH_LINK} -pie -fPIE"
   ./Configure $ARCH
